...
const CartPage = () => {
  ...
  const [orderData] = useCheckoutOrderData(cartItems, profile, calculatedPrices);
  ...
  const getId = (it: (typeof cartItems)[number]) => String(it?.cartLine ?? it?.itemNumber);
  ...
  const [itemsToRemove, setItemsToRemove] = React.useState<Set<string>>(new Set());
  ...
  if (!profile) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-12">
        ...
        <Button onClick={() => setIsAuthOpen(true)}>{t("Login.loginToViewCart")}</Button>
      </div>
    );
  }
        ...
        <div className="space-y-6 md:col-span-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex w-[70%] items-center gap-2">
              <p className="text-base font-normal">{t("Cart.showStockStatus")}</p>
            ...
            <Button variant="outline" className="border-[#C81E1E] text-[#C81E1E]">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" onClick={() => handleArchiveCart()} className="mr-2">
                {t("Cart.archiveCart")}
              </Button>
              <Button variant="outline" onClick={() => router.push("/cart/history")}>
              ...
              return (
                <React.Fragment key={id}>
                  <div
                    ...
                    className={`border-lightGray grid grid-cols-[70px_2fr_2fr_1fr_1fr_40px] items-center gap-4 rounded-md border p-6 mb-4 transition-all duration-300 ease-in-out transform ${
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span className="mb-2 font-medium text-[#0F1912] hover:underline">
                        <Link
                      ...
                      >
                        {item.itemNumber}, 300mm, 3/8” <ChevronRight className="h-4 w-4" />
                      </p>
                    </div>
                    ...
                    <Select
                      onValueChange={async (warehouseNumber: string) => {
                        ...
                        try {
                          await updateWarehouse(item.cartLine ?? 0, item.itemNumber, warehouseNumber);
                        } finally {
                    ...
                    >
                      ...
                      <SelectContent>
                        {warehouseBlance
                          ...
                          ?.warehouses?.map((warehouse) => (
                            <SelectItem key={warehouse.warehouse_number} value={warehouse.warehouse_number}>
                              <div
                                className={`flex items-center justify-center p-0 text-xs ${
                                  warehouse.balance > 0 ? "text-[#009640]" : "text-[#0F1912]"
                                }`}
                              >
                                {warehouse.balance > 0 ? (
                  ...
                  <Modal
                    ...
                    onOpenChange={(open) => setOpenModalId(open ? item.productNumber : null)}
                    className="min-w-[75%]"
                  >
                    <ModalHeader>
