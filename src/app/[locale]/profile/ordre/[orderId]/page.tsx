...
export default function OrderDetails({ params }: { params: { orderId: string } }) {
  ...
  const orderDetails = {
    ...
    summary: {
      originalPrice: "4198,00 kr",
      discount: "-999.00 kr",
      subtotal: "3 199,00 kr",
      vat: "1049,50 kr",
      vatRate: "25%"
    }
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-6">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm mb-6 bg-[#F8F9F8] p-4 rounded">
        <Link href="/profile" className="text-[#5A615D] hover:text-[#0F1912]">
          Mine bestillinger
        </Link>
        <ChevronRight className="h-4 w-4 text-[#8A8F8C]" />
        <span className="text-[#0F1912]">{orderDetails.orderId}</span>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[#E5E7EB] p-6">
        <div className="grid grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <p className="text-[#5A615D]">{orderDetails.address.postalCode} {orderDetails.address.city}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <p className="text-[#5A615D]">Estimert leveringstid: {orderDetails.delivery.estimatedTime}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <p className="text-[#5A615D]">{orderDetails.payment.department}</p>
        ...
        {/* Products */}
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-[#0F1912] mb-4">Dine varer</h2>
          <div className="space-y-4">
            {orderDetails.items.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-4 border border-[#E5E7EB] rounded">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-[#F8F9F8] rounded flex items-center justify-center">
                    <Image src={item.image} alt={item.name} width={48} height={48} />
                  </div>
                  <div>
                    <h3 className="font-medium text-[#0F1912]">{item.name}</h3>
                    <p className="text-sm text-[#5A615D]">{item.sku}</p>
                  </div>
                </div>
                <div className="flex items-center gap-8">
                  <span className="text-sm text-[#5A615D]">x{item.quantity}</span>
                  <span className="font-medium text-[#0F1912]">{item.price}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-8 border-t border-[#E5E7EB] pt-4">
          ...
          <div className="space-y-2 max-w-xs ml-auto">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-[#0F1912]">{orderDetails.summary.originalPrice}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-[#5A615D]">Rabatt</span>
              <span className="text-[#22C55E]">{orderDetails.summary.discount}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-[#5A615D]">Sum etter rabatt (eks. mva.)</span>
              <span className="text-[#0F1912]">{orderDetails.summary.subtotal}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-[#E5E7EB] pt-2">
              ...
              <span className="text-[#0F1912]">{orderDetails.summary.vat}</span>
            </div>
          </div>
        </div>
      </div>
