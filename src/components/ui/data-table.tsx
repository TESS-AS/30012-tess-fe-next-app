							...
							<tbody>
								{data.map((item, index) => (
									<React.Fragment key={index}>
										<tr className="group border-b border-[#C1C4C2] transition-colors duration-200 hover:bg-[#F8F9F8]">
											{isExpandable && (
											...
											{columns.map((column) => (
												<td
													key={column.key}
													className="px-4 py-4 font-medium text-[#0F1912]">
													{column.cell(item)}
												</td>
											))}
									...
									if (shouldShow) {
										return (
											<Button
												...
												variant="outline"
												size="sm"
												className={`h-8 w-8 rounded-none border-r-[0.5px] border-l-0 border-[#8A8F8C] p-0 font-medium text-[#5A615D] ${
