...
export const AppContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [surChargePrices, setSurChargePrices] = useState<Record<string, number>>({});
  const [rabatterPrices, setRabatterPrices] = useState<Record<string, number>>({});
  const [orderSummaryTotalPrice, setOrderSummaryTotalPrice] = useState<Record<string, number>>({});
    ...
    try {
      ...
      for (const item of cart) {
        ...
        setPrices((prev) => ({
          ...prev,
          [item.itemNumber]:
            priceData?.find((p: PriceResponse) => p.itemNumber === String(item.itemNumber))?.basePrice || 0,
        }));
      }
      ...
      if (priceRequests.length > 0) {
        ...
        const newCalculated = priceResults.reduce((acc: Record<string, number>, it: PriceResponse) => {
          acc[it.itemNumber] = it.bestPrice || 0;
          return acc;
        }, {});
        const newSurcharges = priceResults.reduce((acc: Record<string, number>, it: PriceResponse) => {
          acc[it.itemNumber] = it.surCharge || 0;
          return acc;
        }, {});
        const newRabatter = priceResults.reduce((acc: Record<string, number>, it: PriceResponse) => {
  ...
  const updateWarehouse = async (cartLine: number, itemNumber: string, warehouseNumber: string) => {
    try {
  ...
  const removeItemOptimistic = async (cartLine: number) => {
    ...
    setCartItems((prev) => prev.filter((i) => Number(i.cartLine) !== numericLine));
