import { CustomerDimensionItem, UserDimensionItem } from "@/types/dimensions.types";

export function formatCustomerDimensionsToHierarchy(dimensions: CustomerDimensionItem[]): Array<{ label: string; value: string }> {
...
export function formatUserDimensionsToHierarchy(dimensions: UserDimensionItem[]): Array<{ label: string; value: string }> {
    return dimensions.flatMap(dim => {
        const results = [{ label: dim.dimension1.label, value: dim.dimension1.label }];
